...
# Copy everything that is not dockerignored to the image
COPY . .
...
# Final image - Use slim version for smaller image size
...
# Pre-download the Solang compiler image and save it as a tarball
# This is a workaround since we can't use Docker-in-Docker in Vercel
COPY --from=builder /app/crates/backend/src/services/sandbox.rs /app/sandbox.rs
...
CMD ["/app/backend", "--frontend-folder", "/app/packages/frontend/public", "--host", "0.0.0.0", "--port", "9000"]
